...
export function Sidebar() {
    ...
    return (
        <aside className="w-64 glass-card border-r border-black/5 hidden md:flex flex-col p-6 sticky top-0 h-screen shrink-0 relative overflow-hidden z-20">
            ...
            <div className="flex items-center gap-2 mb-10 relative z-10">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-[var(--color-text-primary)] font-bold text-lg shadow-[0_0_15px_rgba(79,141,255,0.4)]">A</div>
                <span className="text-xl font-bold text-[var(--color-text-primary)] tracking-tight font-display">Admin<span className="text-gradient-blue">Panel</span></span>
            </div>
            ...
            <div className="pt-6 border-t border-black/10 relative z-10 w-full">
                <button
                    onClick={handleLogout}
                    className="flex items-center gap-3 w-full px-4 py-3 text-red-400 hover:text-[var(--color-text-primary)] hover:bg-red-500/20 rounded-xl transition-colors text-sm font-medium border border-transparent hover:border-red-500/30"
                >
                    <LogOut size={20} />
                    Sign Out
                </button>
...
function NavItem({ href, icon, label, active = false, badge }: { href: string, icon: any, label: string, active?: boolean, badge?: number }) {
    return (
        <Link href={href} className={`flex items-center justify-between px-4 py-3 rounded-xl cursor-pointer transition-all border ${active ? 'bg-blue-500/10 text-blue-400 border-blue-500/20 shadow-[0_0_15px_rgba(79,141,255,0.1)] font-medium' : 'text-gray-400 border-transparent hover:bg-black/5 hover:text-gray-200 hover:border-black/10'}`}>
            <div className="flex items-center gap-3">
                {icon}
                <span>{label}</span>
            </div>
            {badge && (
                <span className="bg-red-500/80 border border-red-500 text-[var(--color-text-primary)] text-[10px] font-bold px-2 py-0.5 rounded-full min-w-[20px] text-center shadow-[0_0_10px_rgba(239,68,68,0.4)]">
                    {badge}
                </span>
            )}
        </Link>
