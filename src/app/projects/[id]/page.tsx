    ...
    return (
        <main className="min-h-screen bg-[var(--color-bg-primary)] pt-24 pb-24 relative overflow-hidden font-sans">
            ...
            <div className="container mx-auto px-6 relative z-10 max-w-6xl">
                {/* Back Link */}
                <div className="mb-12">
                    <Link href="/#projects" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/[0.05] text-[var(--color-text-muted)] hover:text-white transition-all duration-300 font-mono text-xs font-bold tracking-wider">
                ...
                <div className="flex flex-col md:flex-row gap-8 items-start justify-between mb-16">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 shrink-0 mt-4 md:mt-0">
                        {project.githubLink && (
                            <a href={project.githubLink} target="_blank" rel="noreferrer" className="group flex items-center justify-center gap-3 px-6 py-4 rounded-2xl bg-[#111] hover:bg-[#1a1a1a] border border-white/[0.1] hover:border-white/[0.2] text-white transition-all shadow-xl font-medium">
                                <Github size={20} className="group-hover:rotate-12 transition-transform" /> 
                                <span>Source Code</span>
                            </a>
                        )}
                        {project.link && project.link !== project.githubLink && (
                            <a href={project.link} target="_blank" rel="noreferrer" className="group flex items-center justify-center gap-3 px-6 py-4 rounded-2xl transition-all font-medium text-white shadow-xl hover:shadow-2xl hover:-translate-y-1" style={{ backgroundColor: color, boxShadow: `0 10px 30px -10px ${color}` }}>
                                <span>Live Demo</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-6">
                        ...
                        {/* Featured Image */}
                        {project.image && (
                            <div className="rounded-3xl overflow-hidden border border-white/[0.08] bg-[var(--color-bg-card)] shadow-2xl relative group">
                        ...
                        {details.whyIUsedThis && (
                            <div className="rounded-3xl p-8 md:p-10 border border-white/[0.06] bg-[var(--color-bg-card)] relative overflow-hidden group hover:border-white/[0.1] transition-colors">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <h2 className="text-2xl font-bold text-white">Why I Used This Tech Stack</h2>
                                    </div>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        {/* The Problem */}
                        {details.problem && (
                            <div className="rounded-3xl p-8 border border-white/[0.06] bg-[var(--color-bg-card)] hover:border-white/[0.1] transition-colors">
                                <div className="flex items-center gap-3 mb-6">
                        ...
                        {details.features && details.features.length > 0 && (
                            <div className="rounded-3xl p-8 border border-white/[0.06] bg-gradient-to-b from-[var(--color-bg-card)] to-[var(--color-bg-primary)] flex-1">
                        ...
                        {project.tags && project.tags.length > 0 && (
                            <div className="rounded-3xl p-8 border border-white/[0.06] bg-[var(--color-bg-card)]">
                                <h3 className="text-sm font-mono text-[var(--color-text-muted)] mb-5">TECHNOLOGIES</h3>
                                <div className="flex flex-wrap gap-2">
                                    {project.tags.map((tag: string) => (
                                        <span key={tag} className="px-3 py-1.5 rounded-lg text-sm bg-white/[0.03] border border-white/[0.08] text-[var(--color-text-primary)] hover:border-white/[0.2] transition-colors cursor-default">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
